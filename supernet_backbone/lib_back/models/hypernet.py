...
class SuperNet(nn.Module):
    ...
    def forward_features(self, x, architecture):
        # architecture = [[0], [], [], [], [], [0]]
        print(f'entering forward features')
        x = self.conv_stem(x)
        x = self.bn1(x)
        x = self.act1(x)
        # assert (len(self.blocks == len(architecture)))  # avoid bugs
...
class SuperNet_DP(SuperNet):
    ...
    def build_model(self, model, architecture, stride=16, backbone_index=None):
        print(f'building model')
        model_layers = []
        for layer_idx, (layer, layer_arch) in enumerate(zip(self.blocks, architecture)):
            #print(f'\n\nlayer index: {layer_idx}')
            #print(f'layer: {layer}')
            #print(f'layer_arch: {layer_arch}')
            ...
            for block_idx, (blocks, arch) in enumerate(zip(layer, layer_arch)):
                ...
                #print(f'arch: {arch}')
                #print(f'used module: {blocks[arch]}')
                #model.blocks[layer_idx][0].append(blocks[arch])
                layer_ops.append(blocks[arch])
                ...
                #        return x
            #model.blocks[layer_idx] = nn.Sequential(*layer_ops)
            model_layers.append(nn.Sequential(*layer_ops))
            # idx += 1
        model.blocks = nn.Sequential(*model_layers)
        return model
...
def _gen_supernet(flops_minimum=0, flops_maximum=600, DP=False, **kwargs):
    ...
    if DP:
        #print(f'generating a DP supernet')
