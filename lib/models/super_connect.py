...
from .connect import *
'''2020.09.06 Build a supernet for head'''
...
'''2020.09.06 Point-wise Neck in Mobile setting'''
class Point_Neck_Mobile(nn.Module):
    def __init__(self, inchannels=512, outchannels=256,
                 num_kernel=None, cat=False, BN_choice=None, matrix=False):
        super(Point_Neck_Mobile, self).__init__()

        # encode backbone
        self.cls_encode = matrix_light(in_channels=inchannels, out_channels=outchannels, BN_choice=BN_choice)
        self.reg_encode = matrix_light(in_channels=inchannels, out_channels=outchannels, BN_choice=BN_choice)

        self.cls_dw = GroupPW(num_kernel,cat=cat, matrix=matrix)
        self.reg_dw = GroupPW(num_kernel,cat=cat, matrix=matrix)
    ...
    def forward(self, kernel, search):
        ...
        oup['cls'] = self.cls_dw(cls_z, cls_x)
        '''reg'''
        # encode first
        reg_z, reg_x = self.reg_encode(kernel, search)  # [x11, x12, x13]
        # Point-wise and channel attention
        oup['reg'] = self.reg_dw(reg_z, reg_x)
