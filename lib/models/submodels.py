...
# def get_towers(module_list: torch.nn.ModuleList, path_head, inchannels, outchannels, towernum=8):
#     for tower_idx in range(towernum):
#         block_idx = path_head[1][tower_idx]
#         if tower_idx == 0:
#             if block_idx == 0:
#                 module_list.append(SeparableConv2d_BNReLU(inchannels, outchannels, kernel_size=3,
#                                        stride=1, padding=1, dilation=1))
#             elif block_idx == 1:
#                 module_list.append(SeparableConv2d_BNReLU(inchannels, outchannels, kernel_size=5,
#                                        stride=1, padding=2, dilation=1))
#             else:
#                 raise ValueError()
#         else:
#             if block_idx == 0:
#                 module_list.append(SeparableConv2d_BNReLU(outchannels, outchannels, kernel_size=3,
#                                                         stride=1, padding=1, dilation=1))
#             elif block_idx == 1:
#                 module_list.append(SeparableConv2d_BNReLU(outchannels, outchannels, kernel_size=5,
#                                        stride=1, padding=2, dilation=1))
#     return module_list
...
def build_subnet_head(path_head, channel_list=[128, 192, 256], kernel_list=[3, 5, 0], inchannels=64, towernum=8, linear_reg=False):
    ...
    # add operations
    tower_cls = nn.Sequential(*get_towers(tower_cls_list, path_head['cls'], inchannels, num_channel_cls, towernum=towernum, kernel_list=kernel_list))
    tower_reg = nn.Sequential(*get_towers(tower_reg_list, path_head['reg'], inchannels, num_channel_reg, towernum=towernum, kernel_list=kernel_list))
    # add prediction head
...
class PW_Corr_adj(nn.Module):
    def __init__(self, num_kernel=64, cat=False, matrix=True, adj_channel=128):
        ...
        self.pw_corr = GroupPW(num_kernel, cat=cat, CA=True, matrix=matrix)
