...
class SuperTracker_ablation(Tracker):
    def __init__(self, info, name, effi=0, model=None, cand=None):
        super(SuperTracker_ablation, self).__init__(name=name)
        ...
        self.cand_b, self.cand_h, self.cand_op = cand['back'], cand['head'], cand['ops']
        if self.cand_op is not None:
            if self.cand_op[0] == 1:
                self.stride = 8
